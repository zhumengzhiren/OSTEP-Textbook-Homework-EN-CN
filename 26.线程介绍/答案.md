...
`-i 2`时,最终值为 1,`-i 3`时最终值为 2,`-i 3`的结果是正确，因为算法总共有3个指令, 在临界区被中断时会出现错误。
...
```shell script
./x86.py -p looping-race-nolock.s -a bx=100 -t 2 -M 2000 -i 12 -c
```

使用3的倍数的中断间隔会导致正确结果,因为每次循环需要3个指令,如果中断间隔是3的倍数,则不会在临界区中断,不会出现问题
